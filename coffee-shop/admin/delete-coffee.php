<?php 
    ...
    if(isset($_GET['id']) && isset($_GET['image_name'])) //Either use '&&' or 'AND'
    {
        ...
        if($image_name != "")
        {
            ...
            if($remove==false)
            {
                ...
                //Stop the Process of Deleting coffee
                die();
            }

        }

        //3. Delete  coffee from Database
        $sql = "DELETE FROM tbl_coffee WHERE id=$id";
        //Execute the Query
        $res = mysqli_query($conn, $sql);

        //CHeck whether the query executed or not and set the session message respectively
        //4. Redirect to Manage coffee with Session Message
        if($res==true)
        {
            //coffee Deleted
    ...
    {
        //Redirect to Manage coffee Page
        //echo "REdirect";
